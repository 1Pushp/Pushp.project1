  ...
  return (
    <div
      className={`
        ...
        ${isFocused
          ? 'inset-0 md:inset-4 opacity-100 scale-100 rounded-none md:rounded-lg'
    ...
    >
      ...
      <div className="bg-[#121214] px-3 py-2 md:px-4 md:py-3 flex items-center justify-between border-b border-zinc-800 shrink-0 gap-2">
        {/* Left: Controls */}
        <div className="flex items-center space-x-3 w-auto md:w-32">
        ...
        {/* Right: Actions */}
        <div className="flex items-center justify-end space-x-1 md:space-x-2 w-auto md:w-40">
            {!isLoading && creation && (
                <>
                    {/* Verified Sources Button */}
                    {creation.sources && creation.sources.length > 0 && (
                        <div className="relative">
                            <button
                                onClick={() => setShowSources(!showSources)}
                                className={`p-1.5 rounded-md transition-all flex items-center space-x-1 ${showSources ? 'bg-teal-500/20 text-teal-400' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800'}`}
                            ...
                            {/* Sources Popover */}
                            {showSources && (
                                <div className="absolute top-full right-0 mt-2 w-64 md:w-72 bg-[#121214] border border-zinc-800 rounded-lg shadow-xl z-50 p-3 overflow-hidden">
                                    <div className="flex items-center space-x-2 text-[10px] font-mono uppercase text-zinc-500 mb-3 font-bold border-b border-zinc-800 pb-2">
                                        <GlobeAltIcon className="w-3 h-3" />
                                        <span>Grounding Sources</span>
                                    </div>
                                    <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
                                        {creation.sources.map((source, idx) => (
                                            <a 
                                                key={idx} 
                                                href={source.uri} 
                                                target="_blank" 
                                                rel="noopener noreferrer"
                                                className="block p-2 rounded bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 transition-colors group"
                                            >
                                                <div className="text-xs text-zinc-300 truncate font-medium group-hover:text-teal-400 mb-0.5">{source.title}</div>
                                                <div className="text-[10px] text-zinc-600 truncate font-mono">{source.uri}</div>
                                            </a>
                                        ))}
                                    </div>
                                </div>
                    ...
                    <button 
                        onClick={onReset}
                        title="New Upload"
                        className="ml-1 md:ml-2 flex items-center space-x-1 text-xs font-bold bg-white text-black hover:bg-zinc-200 px-2 py-1 md:px-3 md:py-1.5 rounded-md transition-colors"
                    >
        ...
        {isLoading ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 w-full">
             ...
             <div className="w-full max-w-md space-y-8">
                <div className="flex flex-col items-center">
                    ...
                    <p className="text-zinc-500 text-sm mt-2">Interpreting visual data & Verifying sources...</p>
                </div>
                ...
                <div className="w-full h-1 bg-zinc-800 rounded-full overflow-hidden">
                 ...
                 {/* Terminal Steps */}
                 <div className="border border-zinc-800 bg-black/50 rounded-lg p-4 space-y-3 font-mono text-sm hidden sm:block">
                     <LoadingStep text="Analyzing visual inputs" active={loadingStep === 0} completed={loadingStep > 0} />
                     <LoadingStep text="Search Grounding: Verifying data" active={loadingStep === 1} completed={loadingStep > 1} />
          ...
          <>
            ...
            {showSplitView && creation.originalImage && (
                <div className="w-full md:w-1/2 h-[30%] md:h-full border-b md:border-b-0 md:border-r border-zinc-800 bg-[#0c0c0e] relative flex flex-col shrink-0">
                    <div className="absolute top-2 left-2 md:top-4 md:left-4 z-10 bg-black/80 backdrop-blur text-zinc-400 text-[10px] font-mono uppercase px-2 py-1 rounded border border-zinc-800">
                        Input Source
                    </div>
                    <div className="w-full h-full p-4 md:p-6 flex items-center justify-center overflow-hidden">
                        {creation.originalImage.startsWith('data:application/pdf') ? (
                            ...
                            <div className="relative w-full h-full flex items-center justify-center bg-black/20 rounded-lg border border-zinc-800/50 p-1">
                                {/* Custom grid background for video player empty space */}
                                ...
                                <video 
                                    ...
                                    controls 
                                    className="relative z-10 max-w-full max-h-full rounded bg-black shadow-2xl outline-none"
                                >
            ...
            {/* App Preview Panel */}
            <div className={`relative h-full bg-white transition-all duration-500 ${showSplitView && creation.originalImage ? 'w-full md:w-1/2 h-[70%] md:h-full' : 'w-full'}`}>
                 <iframe
