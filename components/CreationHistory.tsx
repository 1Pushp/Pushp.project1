...
import { ClockIcon, ArrowRightIcon, DocumentCheckIcon, PhotoIcon, VideoCameraIcon } from '@heroicons/react/24/outline';
  ...
  return (
    <div className="w-full animate-in fade-in slide-in-from-bottom-8 duration-700">
      <div className="flex items-center space-x-3 mb-3 px-2">
        ...
        <h2 className="text-xs font-bold uppercase tracking-wider text-zinc-500">Verification History</h2>
      ...
      <div className="flex overflow-x-auto space-x-4 pb-2 px-2 scrollbar-hide">
        {history.map((item) => {
          ...
          const isVideo = item.originalImage?.startsWith('data:video');

          return (
            <button
              key={item.id}
              onClick={() => onSelect(item)}
              className="group flex-shrink-0 relative flex flex-col text-left w-44 h-28 bg-zinc-900/50 hover:bg-zinc-800 border border-zinc-800 hover:border-teal-500/30 rounded-lg transition-all duration-200 overflow-hidden"
            >
              <div className="p-4 flex flex-col h-full">
                ...
                <div className="mt-auto">
                  ...
                  <div className="flex items-center space-x-1 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <span className="text-[10px] text-teal-400">View Report</span>
                    <ArrowRightIcon className="w-3 h-3 text-teal-400" />
                  </div>
                </div>
              </div>
            </button>
